...
const device_id = `${Device.model()}_${Device.name()}`
  .replace(/[^a-zA-Z0-9\-\_]/, "")
  .toLowerCase();
...
exports.generateSlices = async function ({ caller = "none" }) {
  ...
  const homeSize =
    homeSizes[
      await presentAlert(
        "Is your home screen set to small, or large?",
        ["Small (default)", "Large (no text)"],
        ALERTS_AS_SHEETS,
      )
    ];
...
exports.getSlice = async function (name) {
  ...
  const imgPath = fm.joinPath(
    cachePath,
    `${device_id}-${appearance}-${position}.jpg`,
  );
  if (!fm.fileExists(imgPath)) {
    log("image does not exists. setup required.");
    var setupCompleted = await exports.generateSlices({
      caller: "getSliceForWidget",
    });
    if (!setupCompleted) {
      return null;
    }
...
exports.getSliceForWidget = async function (instance_name, reset = false) {
  ...
  if (!position) {
    ...
    const testImage = fm.joinPath(
      cachePath,
      `${device_id}-${appearance}-medium-top.jpg`,
    );
...
//------------------------------------------------
async function presentAlert(prompt = "", items = ["OK"], asSheet = false) {
  let alert = new Alert();
...
if (module_name == Script.name()) {
  await (async () => {
    let opts = ["Generate Slices", "Clear Widget Positions Cache", "Cancel"];
